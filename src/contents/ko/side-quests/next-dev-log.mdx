...
- 한영 전환
  해외에서 일하고 싶다는 꿈도 있지만, 공식 영문 문서에 더 친숙해지기 위해 도전했다.

- 디자인에 개인 취향을 듬뿍 담을 것
  이 블로그를 포트폴리오로도 활용할 예정이라 프론트엔드 개발자로서의 보는 재미도 한 방울 첨가하고 싶었다. 개인적인 취향을 가득 담았다. 취향이 바뀌거나 도전하고 싶은 디자인이 생기면 그때마다 적용해볼 것이다.
...
export function getMetaData(locale: string): PostMeta[] {
    ...
    // getMetaData 인자로 언어를 건네서 그에 맞는 포스트를 가져온다.
