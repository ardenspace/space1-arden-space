...
ports: - "3001:80"
// 👆👆 서버 nginx가 3001로 요청 보냄 > 도커 컨테이너의 80포트로 전달 > 80 포트로 들어온 요청 처리
environment: - NODE_ENV=development
restart: unless-stopped
networks: - 'myprojectname'-dev-network # 볼륨 마운트로 소스 코드 변경사항 실시간 반영
volumes: - .:/app - /app/node_modules
...
nginx에서 .conf 파일은 nginx의 동작을 정의하는 설정 파일이다. 어떤 요청을 어떻게 처리할지 알려주는 파일이라고 할 수 있다. nginx와 docker 사이에 이어지는 흐름을 먼저 정리하자면 다음과 같다.
...
외부 요청 (배포할 사이트.com)
🔽
서버의 nginx (nginx 서버 내.conf) - 포트 80에서 수신
🔽
proxy_pass http://127.0.0.1:3001 (Docker 컨테이너로 프록시)
🔽
Docker 컨테이너 내부의 nginx (Dockerfile.dev) - 포트 80에서 수신
🔽
...
기존에는 정적 파일을 nginx가 직접 서빙했지만 이제는 Docker container와 이어주는 다리 역할을 한다. nginx는 외부 요청을 해당 도커 컨테이너에 연결해주는 역할을 맡았으므로 proxy_pass 설정을 수정해 주어야 한다.
