...
<br />
The problem is… things are never not busy. There’s always something going on at the
company, and after work I’m tied up with side projects. So I never even got the chance
to try.
...
Git, and we’d just build directly on the server.
<br />A few days after hooking up Git, though, my team lead came by and said
he’d set up a deployment environment — and suggested we integrate CI/CD into one
of the sites I’m currently managing.
...
beforehand helps a lot when writing code — you’re less likely to get lost. I didn’t
know that at first and just dove in headfirst… which meant I ended up fumbling around
quite a bit. 😅
...
push it up. So that’s what I did. Slow and steady. Slow and steady!
...
runs-on: act-runner-1 // 👈👈 Default runner added by Gitea Actions / more on this later
...
ports: - "3001:80"
// 👆👆 Server Nginx sends requests to port 3001 > forwarded to port 80 in the Docker container > handled by Nginx inside the container
environment: - NODE_ENV=development
restart: unless-stopped
networks: - 'myprojectname'-dev-network # Reflect source code changes in real time using volume mounts
volumes: - .:/app - /app/node_modules
    ...
    steps:
      ...
      - name: Build and Deploy
        run: |
          ...
          # 👈👈 Same process as local, just switch to the dev yml file
          docker rm -f 'myprojectname'-dev || true
...
Until I finally witnessed this glorious sea of green lights, I had to endure countless red ones . . . 😭😭😭😭😭

<br />
## Editing the nginx conf File

In nginx, a .conf file defines how nginx operates. In other words, it tells nginx how to handle incoming requests. Before diving into the details, let’s first outline the flow between nginx and Docker:
...
🔽
Server-side nginx (.conf inside the nginx server) — listens on port 80
🔽
proxy_pass http://127.0.0.1:3001 (forwarded to the Docker container)
🔽
nginx inside the Docker container (Dockerfile.dev) — listens on port 80
🔽
...
<br />
It was really, really fun. Not only was I happy to work on something I’d been eager to try, but I also enjoyed tinkering freely with the server — something I usually don’t get to touch. Sharing the process with my teammates at the end felt really rewarding and joyful.
<br />
I ran into many errors along the way, and I’ll cover the solutions in detail later. I also plan to write about integrating CI/CD for our mobile app.
<br />
<br />
To anyone reading this: thank you! I sincerely hope this was helpful. ☺️☺️
